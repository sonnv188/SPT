...
public class ParcelInsertionPeopleDirectServiceGreedy implements OnlineParcelInsertion {

	public SimulatorTimeUnit sim;
	public PrintWriter log;
	public SequenceOptimizer seqOptimizer = null;
	
	public ParcelInsertionPeopleDirectServiceGreedy(SimulatorTimeUnit sim){
		this.sim = sim;
		this.log = sim.log;
		seqOptimizer = new SequenceOptimizer(sim, sim.maxPendingStops + 10);		
	}
	public String name(){ return "ParcelInsertionPeopleDirectServiceGreedy";}
	...
	public TaxiTimePointIndex findTaxiForParcelInsertion(ParcelRequest pr, double maxTime){
		...
		for(int i = 0; i < sim.vehicles.size(); i++){
			...
			if(t > maxTime){
				System.out.println(name() + "::findTaxiForParcelInsertion, i = "+ i + ", t = " + t + " EXPIRED maxTime = " + maxTime);
				break;
			}
		}
	...
	public ServiceSequence computeParcelInsertionSequence(Vehicle taxi,
		...
		int[] t_sel_nod = seqOptimizer.computeShortestSequence(taxi, tpi, keptReq, r, maxTime);
	...
	public void processParcelRequests(ArrayList<ParcelRequest> parReq, double startDecideTime) {
		...
		for(int i = 0; i < parReq.size(); i++){
			...
			//if((System.currentTimeMillis()-startDecideTime)*0.001 > sim.TimePointDuration){sim.nbParcelRejects++;continue;}
