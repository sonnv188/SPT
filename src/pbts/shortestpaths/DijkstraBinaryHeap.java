...
public class DijkstraBinaryHeap {
	...
	private long composeKey(int u, int v){
		return (long)u * (long)cooe + (long)v;
	}
	...
	*/
	...
	public String name(){ return "DijkstraBinaryHeap";}
	public Itinerary queryShortestPath(int s, int t){
		Itinerary I = solve(s,t);
		if(I != null){
			long key = composeKey(s,t);
			//long lkey = (long)s * (long)cooe + (long)t;
			//System.out.println(name() + "::queryShortestPath(" + s + "," + t + ", key = " + key);
	...
	public boolean distanceAvailable(int s, int t){
		long key = composeKey(s,t);
		return mCost.get(key) != null;
	}
